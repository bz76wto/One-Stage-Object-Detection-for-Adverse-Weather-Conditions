...
from utils.general import (check_file, check_img_size, check_imshow, check_requirements, colorstr, check_version, 
...
class DetectTensorRT(nn.Module):
    # YOLOv5 MultiBackend class for python inference on various backends
    def __init__(self, weights='yolov5s.engine', device=torch.device('cpu'), data=None, fp16=False):
        super().__init__()
        ...
        check_version(trt.__version__, '7.0.0')  # require tensorrt>=7.0.0
...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    model = DetectTensorRT(weights, device=device, data=data, fp16=half)
